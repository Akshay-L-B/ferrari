...
const Slug=()=>{
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await fetch(`/api/allEventrequestadditions?instructor=${slug}`);
  ...
  const handleApprove = async () => {
    try {
      ...
      const res = await fetch(`/api/approveEventAddition?EventID=${selectedEventID}`, {
        method: 'PUT',
        headers: {
      ...
      if (res.ok) {
        toast.success(`${selectedEventID} has been approved for the faculty ${slug}!`, {
          position: 'top-center',
          autoClose: 1500,
        ...
        const res1 = await fetch(`/api/assignEventtofaculty?EventID=${selectedEventID}&FacultyID=${slug}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
                      ...
                      {filteredData.map((Event) => (
                        <tr key={Event.EventID}>
                            ...
                            <div className="flex items-center">
                                <p className="flex items-center justify-center w-6 h-6 -mx-1 text-s text-blue-600">{new Date(Event.EventRegDeadline).toISOString().split('T')[0]}</p>
                            </div>
                            </td>
                            <td className="px-8 py-4 text-sm whitespace-nowrap dark:bg-gray-900">
                            <div className="flex items-center">
                                <p className="flex items-center justify-center w-6 h-6 -mx-1 text-s text-blue-600">{new Date(Event.EventDropDeadline).toISOString().split('T')[0]}</p>
                            </div>
                            </td>
                            <td className="px-8 py-4 text-sm whitespace-nowrap dark:bg-gray-900">
                            <div className="flex items-center">
                                <p className="flex items-center justify-center w-6 h-6 -mx-1 text-s text-blue-600">{Event.NoLectures}</p>
                            </div>
                            </td>
                            <td className="px-8 py-4 text-sm whitespace-nowrap dark:bg-gray-900">
                            <div className="flex items-center">
                                <p className="flex items-center justify-center w-6 h-6 -mx-1 text-s text-blue-600">{Event.NoPracticals}</p>
                            </div>
                            </td>
                            <td className="px-8 py-4 text-sm whitespace-nowrap dark:bg-gray-900">
  {Event.prerequisites.map((Event, index) => (
    <div key={index} className="flex items-center space-x-2">
      <p className="flex items-center justify-center w-6 h-6 -mx-1 text-s text-blue-600">
        {Event.prerequisitesBranch}
      </p>
      <p className="flex items-center justify-center w-6 h-6 -mx-1 text-s text-blue-600">
        -{Event.prerequisitesDegree}
      </p>
    </div>
  ))}
</td>
